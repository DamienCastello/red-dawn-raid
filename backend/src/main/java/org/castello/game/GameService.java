...
public class GameService implements RaidFlow {
    ...
    public GameService(GameStore store, Dice dice, DeckService decks, WeatherService weather,
            ...
            LocationEffectService locationEffects,
     ...
     */
    // --- Équipement : délégués vers EquipmentService ---
    private int currentWeaponTier(Player p) { return equipment.currentWeaponTier(p); }

    private int currentArmorTier(Player p) { return equipment.currentArmorTier(p); }

    private boolean hasResourcesForEquipment(Player p, String equipCode) { return equipment.hasResourcesForEquipment(p, equipCode); }

    private void payResourcesForEquipment(Player p, String equipCode) { equipment.payResourcesForEquipment(p, equipCode); }

    private boolean canForgeAnything(Game g, Player p) { return equipment.canForgeAnything(g, p); }

    private java.util.List<String> listForgeOptionsForPlayer(Game g, Player p) { return equipment.listForgeOptionsForPlayer(g, p); }

    private void applyForge(Game g, Player p, String equipCode) { equipment.applyForge(g, p, equipCode); }

    private String labelEquipmentFr(String equipCode) { return equipment.labelEquipmentFr(equipCode); }

    private void rebuildEquipmentMods(@NonNull Game g) { equipment.rebuildEquipmentMods(g); }

    private int bleedBonusForWeapon(String weaponCode) { return equipment.bleedBonusForWeapon(weaponCode); }

    private int stunPenalityForWeapon(String weaponCode) { return equipment.stunPenalityForWeapon(weaponCode); }

    private boolean rangedKeepAwayTriggered(String weaponCode, int rawAtk) { return equipment.rangedKeepAwayTriggered(weaponCode, rawAtk); }

    private int vampRegenAmount(String weaponCode, int attackRoll) { return equipment.vampRegenAmount(weaponCode, attackRoll); }

    private void convertHunterGearToServant(Game g, Player p) { equipment.convertHunterGearToServant(g, p); }

    private int vampArmorDefenseBonus(String armorCode) { return equipment.vampArmorDefenseBonus(armorCode); }

    private boolean hunterArmorIsAntiBite(String armor) { return equipment.hunterArmorIsAntiBite(armor); }

    private boolean vampArmorHasEvasion(String armorCode) { return equipment.vampArmorHasEvasion(armorCode); }

    private boolean hunterWeaponHasBleed(String w) { return equipment.hunterWeaponHasBleed(w); }

    private boolean hunterWeaponHasStun(String w) { return equipment.hunterWeaponHasStun(w); }

    private boolean hunterWeaponIsRanged(String w) { return equipment.hunterWeaponIsRanged(w); }

    private boolean vampWeaponHasRegen(String w) { return equipment.vampWeaponHasRegen(w); }

    private int hunterWeaponTier(String weaponId) { return equipment.hunterWeaponTier(weaponId); }

    private int hunterArmorTier(String armorId) { return equipment.hunterArmorTier(armorId); }

    private String merchantWeaponIdForTier(int tier) { return equipment.merchantWeaponIdForTier(tier); }

    private String merchantArmorIdForTier(int tier) { return equipment.merchantArmorIdForTier(tier); }

    private int vampireWeaponTier(String weaponId) { return equipment.vampireWeaponTier(weaponId); }

    private int vampireArmorTier(String armorId) { return equipment.vampireArmorTier(armorId); }

    private String vampireWeaponIdForTier(int tier) { return equipment.vampireWeaponIdForTier(tier); }

    private String vampireArmorIdForTier(int tier) { return equipment.vampireArmorIdForTier(tier); }
